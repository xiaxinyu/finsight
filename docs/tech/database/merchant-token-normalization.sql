-- FinSight merchant token normalization (manual execution).
-- Mirrors Java com.finsight.application.analytics.MerchantNormalizer.
-- Run against your application database BEFORE relying on drilldown / analytics parity.
--
--   mysql -u ... -p finsight < docs/tech/database/merchant-token-normalization.sql
--
-- After applying:
...
-- Verification: profile tokens with zero matching analytics rows (should be empty after fix).
SELECT
    mp.user_id,
    mp.merchant_token,
    mp.display_name,
